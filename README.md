...
| 🌐 **QuickTrans** | `Ctrl+Alt+T`. A tab in the same window: translate the selection with several engines at once — MyMemory, DeepL, Claude, OpenAI. Press `1`–`9` to insert one. The menu stays beside it, so you can translate, insert, then run a menu action without leaving. MyMemory needs no API key. |
